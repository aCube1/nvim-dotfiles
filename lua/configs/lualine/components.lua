...
return {
	...
	lsp = {
		function(msg)
			...
			local language_servers = ("[%s]"):format(table.concat(unique_client_names, ", "))
	...
	scrollbar = {
		function()
			...
			local index = math.ceil(line_ratio * #chars) -- line_ration * chars.len() -> Integer
			return chars[index]
		end,
		padding = { left = 0, right = 0 },
		color = "SLProgress",
